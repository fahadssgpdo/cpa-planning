...
1. Provision private App Storage so `DEFAULT_OBJECT_STORAGE_BUCKET_ID` and `PRIVATE_OBJECT_DIR` are available.
...
Deploy the previous application version before reverting any schema. Leave the new nullable columns in place during rollback; they are backward compatible and preserve the object keys needed to recover uploaded files. Do not delete App Storage objects or drop storage metadata columns until the replacement version is stable and all required files have been retained elsewhere.