...
export default function KnowledgeBase() {
  ...
  /* ── Documents data ── */
  const docQueryParams = docCategory === "all" ? {} : { category: docCategory };
  ...
  const handleCreateGlossary = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    createGlossaryMutation.mutate({
      data: {
        termAr: formData.get("termAr") as string,
        termEn: (formData.get("termEn") as string) || undefined,
        definition: formData.get("definition") as string,
        examples: (formData.get("examples") as string) || undefined,
      }
    });
  };
  ...
  const filteredGlossary = (glossaryEntries ?? []).filter(g =>
    g.termAr.toLowerCase().includes(search.toLowerCase()) ||
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2 border-b pb-0">
        ...
        <button
          onClick={() => { setMainTab("glossary"); setSearch(""); }}
          className={`flex items-center gap-2 px-4 py-2.5 text-sm font-medium border-b-2 transition-colors ${
            mainTab === "glossary"
              ? "border-primary text-primary"
              : "border-transparent text-muted-foreground hover:text-foreground"
      ...
      {mainTab === "documents" && (
        <>
          <div className="flex flex-col gap-4">
            <Tabs value={docCategory} onValueChange={setDocCategory} className="w-full">
          ...
          <div className="space-y-3">
            {glossaryLoading ? (
              ...
              <div className="text-center py-12 bg-card rounded-xl border border-dashed">
                ...
                <h3 className="text-lg font-medium text-muted-foreground">{t.glossary.noEntries}</h3>
              </div>
            ) : (
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          {entry.termEn && (
                            <span className="text-sm text-muted-foreground font-medium border-s ps-3">
                              {entry.termEn}
                            </span>
                          )}
                        </div>
